...
export async function executeCreateLabelFlow(opts) {
    ...
    try {
        ...
        if (adapter.capabilities.includes(Capabilities.CREATE_SHIPMENT)) {
            ...
            const shipmentRes = await adapter.createShipment({ shipment, credentials }, context);
        ...
        if (adapter.capabilities.includes(Capabilities.CREATE_PARCEL)) {
            const shipmentCarrierId = result.shipmentResource?.carrierId || shipment.id;
            for (const parcel of parcels) {
                context.logger?.debug("Flow: Creating parcel", {
                    parcelId: parcel.id,
                    weight: parcel.weight,
                });
                const parcelRes = await adapter.createParcel(shipmentCarrierId, { shipment, parcel, credentials }, context);
        ...
        if (adapter.requires?.createLabel?.includes(Capabilities.CLOSE_SHIPMENT) &&
            ...
            const closeRes = await adapter.closeShipment(shipmentCarrierId, context);
            if (store) {
                await store.appendEvent(shipment.id, {
        ...
        if (adapter.capabilities.includes(Capabilities.CREATE_LABEL)) {
            for (let i = 0; i < result.parcelResources.length; i++) {
                ...
                const labelRes = await adapter.createLabel(parcelRes.carrierId, context);
