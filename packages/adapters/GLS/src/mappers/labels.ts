...
import {
  ...
  type CreateParcelsGLSCarrierOptions,
} from "./parcels.js";

function mapGLSErrorCategory(
  errorCode: number | string | undefined,
): "Validation" | "Auth" | "Permanent" | "Transient" | "NotFound" {
...
function resolveLabelContentType(
  printerType?: string,
): "application/pdf" | "application/x-zpl" {
  return resolveLabelFormat(printerType) === "ZPL"
    ? "application/x-zpl"
    : "application/pdf";
}
...
export function mapGLSPrintLabelsToCanonicalCreateLabels(
  ...
  const successfulLabels =
    glsResponse.printLabelsInfoList ||
    (glsResponse as any).PrintLabelsInfoList ||
    [];
  const errors =
    glsResponse.printLabelsErrorList ||
    (glsResponse as any).PrintLabelsErrorList ||
    [];
  ...
  const failedResults = errors.map((error: any, idx: number) => ({
    inputId: (error.clientReferenceList?.[0] ||
      error.ClientReferenceList?.[0] ||
      error.parcelIdList?.[0] ||
      error.ParcelIdList?.[0] ||
      `error-${idx}`) as string,
  ...
  const allResults = [
    ...
    ...successfulLabels.map((label: any, idx: number) => ({
      inputId:
        ...
        label.ClientReference ||
        String(label.parcelId || label.ParcelId),
...
export function mapCanonicalCreateLabelsToGLSGetPrintData(
  ...
  const glsParcels: GLSParcel[] = req.parcelCarrierIds.map(
    (parcelCarrierId, idx) => ({
...
export function mapGLSGetPrintDataToCanonicalCreateLabels(
  ...
  const successfulLabels =
    glsResponse.printDataInfoList ||
    (glsResponse as any).PrintDataInfoList ||
    [];
  const errors =
    glsResponse.getPrintDataErrorList ||
    (glsResponse as any).GetPrintDataErrorList ||
    [];

  // Convert PDF bytes to Buffer or Uint8Array
  let pdfBuffer: Buffer | Uint8Array | undefined;
  const pdfDocument =
    glsResponse.pdfdocument || (glsResponse as any).Pdfdocument;
  if (pdfDocument) {
...
export function mapGLSGetPrintedLabelsToCanonicalCreateLabels(
  ...
  const successfulLabels =
    ...
    [];
  const errors =
    glsResponse.getPrintedLabelsErrorList ||
    (glsResponse as any).GetPrintedLabelsErrorList ||
    [];
